...
{
    /// <summary>
    /// Can trigger audio when a target enters the collider in 2D
    /// </summary>
    [RequireComponent(typeof(Collider2D))]
    public class AudioTrigger2D : Trigger2D
    {
        [System.Serializable]
        ...
        void Start()
        {
            if (collider== null)
            {
                UnityEngine.Debug.LogError($"The {typeof(AudioTrigger2D)} must have a {typeof(Collider2D)} reference for field 'Collider' or it may not work properly!");
                return;
            }
            ...
            if (triggeredBackgroundAudio.Length > 0)
            {
                foreach (var backgroundAudio in triggeredBackgroundAudio)
                {
                    ...
                    {
                        UnityEngine.Debug.LogError($"The {typeof(AudioTrigger2D)} {gameObject.name} has a triggered background audio that has a null {typeof(BackgroundAudioSO)}");
                        return;
                    }
        ...
        /// <returns></returns>
        private bool IsColliderRightOfTrigger(Collider2D collider) => Mathf.Sign(collider.transform.position.x - this.collider.bounds.center.x) == 1;
        ...
        {
            UnityEngine.Debug.Log("Called enable audio!");
        ...
        {
            UnityEngine.Debug.Log("Called disable audio!");
